...
!  
! Analytic:
! apfDataIn   Input data, not altered
! apfData     Current analytic potential data
! apfDataOpt  Optimum
! Spline:
! splineNodesKey, splineNodesData
! splineNodesKeyOpt, splineNodesDataOpt
!
! Actual tabulated potentials
! eamData, eamKey
! eamDataOpt, eamKeyOpt
! 
...
! ------------------------------------
! Print header for Optimise section
! ------------------------------------
...
! If spline/tabulated input potential, make potential from spline nodes
    If(potentialType.eq.1)Then  ! spline/tabulated
      Call setEamSpline(.true.) ! readEAM.f90 setEamSpline(calcDerivIn)  true/false
      splineNodesDataOpt = splineNodesData ! Save optimum
    End If  
! Run calculation for input potential
    Print *,""
    Print *,""
    print *,"==================================================================="
...
! Output Bulk Properties   
    If(potentialType.eq.1)Then  ! spline/tabulated 
      Call runCalcs(1,1,.false.)       ! runCalcs(splineNodesIn,runTypeIn,eosChartIn)
    End If  
    If(potentialType.eq.2)Then  ! Analytic
      Call runCalcs(0,1,.false.)       ! runCalcs(splineNodesIn,runTypeIn,eosChartIn)
    End If        
    Call outputBpT() 
! store as optimum
    optimumRSS = totalRSS
! Save input potential charts/graphs
    Call eamCharts("P_01_input")          ! readEAM.f90
    
    
! If analytic, go through random vary subroutine first
    If(potentialType.eq.2)Then  ! analytic
      apfDataOpt = apfData ! Save optimum
      Call randAnVar()
    End If  
    
    Do i=1,saConfigIn(1)%saCount
      saConfigLive = saConfigIn(i)
      print *,""
      print *,""
      print *,"==================================================================="
      print *,"                 Simulated Annealing Loop: ",i
      If(saConfigIn(i)%saType.eq.1)Then
        print *,"                        Spline/Tabulated "
      End If
      If(saConfigIn(i)%saType.eq.2)Then
        print *,"                          Analytic "
      End If
      print *,"==================================================================="
      print *,""
...
! Make spline    
    If(splineNodes.eq.1)Then
      Call setEamSpline(.true.)  ! readEAM.f90 setEamSpline(calcDerivIn)
    End If        
! Turn off verbose    
    quietOverride = .true.     
...
! ---------------------------------------------------------------------------------------------------  
!
! Random Analytic Variations
!  
! ---------------------------------------------------------------------------------------------------
  Subroutine randAnVar()
! Randomly vary analytic variables within user input boundaries
    Implicit None   ! Force declaration of all variables
! Private variables
    Integer(kind=StandardInteger) :: i
    ...
    If(TerminalPrint())Then
      print *,"==================================================================="
      print *,"        Randomly Vary Analytic Parameters                          "
      print *,"==================================================================="
    End If
    
    If(anOptRandLoops.gt.0)Then
      Do i=1,anOptRandLoops  
        Call varyAnalyticRandom(apfDataOpt)
        Call runCalcs(0,1,.false.)    ! runCalcs(splineNodesIn,runTypeIn,eosChartIn)
        If(totalRSS.lt.optimumRSS)Then
          print *,i,totalRSS,"*","(",maxDensity,")"
          optimumRSS = totalRSS
          apfDataOpt = apfData
        Else  
          print *,i,totalRSS,"(",maxDensity,")"
