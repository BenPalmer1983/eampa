...
!declare global variables  
  ...
  Real(kind=SingleReal), Dimension( : , : ), Allocatable :: eamData
...
!store/output potential count etc
	...
	write(999,"(A28,A1)") "Calculation run type:       ",calcRunType
    ...
    if(saveFilePot.eq."Y")then
	  do i=1,size(eamKey)/5
		...
		do j=eamKey(i,4),(eamKey(i,4)+eamKey(i,5)-1)
		  write(21,"(E16.8E3,A2,E16.8E3,A4,I8,I8,I8)") eamData(j,1),"  ",&
		  eamData(j,2),"    ",i,k,j
		  k = k + 1
		enddo
      enddo
