...
!mpi variables
	...
	Integer(kind=StandardInteger), Dimension( : , : ), Allocatable :: configAtomsMapTemp
...
!-----------------------------------------------------------
    configAtomsMapTemp = configAtomsMap
	!Call extractArrayColumnInt(configAtomsMapTemp,processMap,21) 
	!Call MPI_distributeArray(configurationEnergy,processMap)
...
!Only calculate bulk modulus where there is a reference BM
    ...
    End Do
	Call synchMpiProcesses() !Synchronise mpi processes at this point
	!Call extractArrayColumnInt(configAtomsMap,outputArray,1) 
!--------------------------------------------------
! configurationEquVolume
!--------------------------------------------------
	Allocate(bufferArrayA(1:configCount))
	Allocate(bufferArrayB(1:configCount))
    If(mpiProcessID.gt.0) Then
!send buffers from all worker processes
      Do i=1,(mpiProcessCount-1)
        If(i.eq.mpiProcessID)Then
          processTo = 0
          tagA = 20000 + mpiProcessID	
		  bufferArraySize = configCount
          Call MPI_send(configurationEquVolume,bufferArraySize,&
		  MPI_double_precision,processTo,tagA,MPI_comm_world,error)
          processTo = 0
          tagB = 21000 + mpiProcessID	
		  bufferArraySize = configCount
          Call MPI_send(configurationEquLat,bufferArraySize,&
		  MPI_double_precision,processTo,tagB,MPI_comm_world,error)
	    End If
      End Do
    End If
!read buffers from worker processes
...
!read buffers from worker processes
      Do j=1,(mpiProcessCount-1)
!receive array configurationEquVolume
		processFrom = j
        tagA = 20000 + j
		bufferArraySize = configCount
        Call MPI_recv(bufferArrayA,bufferArraySize,&
		MPI_double_precision,processFrom,tagA,MPI_comm_world,status,error)
!receive array configurationEquLat
		processFrom = j
        tagB = 21000 + j
		bufferArraySize = configCount
        Call MPI_recv(bufferArrayB,bufferArraySize,&
		MPI_double_precision,processFrom,tagB,MPI_comm_world,status,error)
!Collate arrays
		processFrom = j
		Do i=1,configCount
		  If(configAtomsMap(i,24).eq.processFrom)Then
		    configurationEquVolume(i) = bufferArrayA(i)
		    configurationEquLat(i) = bufferArrayB(i)
		  End If
		End Do
      End Do
	End If  
!send array from master to worker processes
	Call MPI_sendout(configurationEquVolume,"double")	
	Call MPI_sendout(configurationEquLat,"double")		
...
!-----------------------------------------
	Allocate(bufferArray(1:configCount))
	sendArray = bufferArray
	Do j=1,3
      If(mpiProcessID.gt.0) Then
!send buffers from all worker processes
        Do n=1,(mpiProcessCount-1)
          If(n.eq.mpiProcessID)Then
            processTo = 0
            tag = 1500 + mpiProcessID	
		    bufferArraySize = configCount
            Call MPI_send(configurationBM,bufferArraySize,&
...
!open forces output file	
	  ...
	  !close(999)	
	!End If
	!print *,mpiProcessID,trialResidualSquareSum
	
	!print *,""
	!print *,"Difference calc configuration ",1	
	!print *,""
    !Do i=1,configCount
	!print *,configAtoms(i),configurationEnergy(i),configurationRefEnergy(i),energyDifference
	!End Do
	!print *,""
