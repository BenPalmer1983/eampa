...
IF(r .LT. 0.0D0)THEN
  dydr = 0.0D0
ELSE
  dydr = 0.5D0 * r**(-0.5D0) + 2.0D0 * p(1) * (r) + 4.0D0 * p(2) * (r)**3
END IF
END SUBROUTINE ackland_embedding_grad


SUBROUTINE ackland_embedding_grad_v(r, p, p_fixed, dydr)
!############################################################
! f(x) = A * sqrt(r) + B * r**2 + C * r**4
IMPLICIT NONE
!############################################################
...
!############################################################
DO n = 1, SIZE(r,1)
  CALL ackland_embedding_grad(r(n), p,  p_fixed, dydr(n))
END DO
END SUBROUTINE ackland_embedding_grad_v
