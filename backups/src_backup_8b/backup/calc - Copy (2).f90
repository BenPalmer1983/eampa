...
!prepare set of data points used in interp
		...
		End Do
		Allocate(interpPointsTemp(1:potLength,1:2))
		Do k=1,potLength
		  interpPointsTemp(k,1) = eamData(k+potStart-1,1)
		  interpPointsTemp(k,2) = eamData(k+potStart-1,2)
		End Do
		
!calculate y and y' from interp points
	    yArray = PointInterpolationFull(interpPoints,x)
		!y = PointInterpolation(interpPointsTemp,x,interpPointCount)
		y = PointInterpolation(eamData,x,interpPointCount,potStart,potLength)
		!y = PointInterpolation(eamData,x,interpPointCount,potStart,&
        !potLength)
		!y = PointInterpolation(interpPoints,x,interpPointCount)
		!y = PointInterpolation(eamData,x,interpPointCount)
		print *,x,yArray(1),y,1,5,potStart,potLength
		!
