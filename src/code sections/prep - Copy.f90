...
!Internal subroutine variables
	...
	Integer(kind=StandardInteger) :: atomA, atomB, atomAKey, atomBKey, nlKey
	...
	Integer(kind=TinyInteger), Dimension(1:10000000) :: nlUniqueKeys
    ...
    Real(kind=DoubleReal), Dimension( : , : ), Allocatable :: neighbourListCoordsTemp
...
!Allocate temp neighbour list array - assume 100 neighbours per atom
	...
	Allocate(neighbourListITemp(1:tempNeighbourListLength,1:6))
...
!Check if unique pair
                          If(nlUniqueKeys(nlKey).eq.0)Then
                            ...
                            print *,atomA,atomB,nlKey,nlUniqueKeys(nlKey)							
...
!close output file
    close(999)	
!Save neighbour list
    If(saveFileNeighbourList.eq."Y")Then
	  outputFile = trim(currentWorkingDirectory)//"/output/"//"nl.dat"
	  Open(unit=11,file=trim(outputFile))	
	  Write(11,"(A8,A8,A8,A8,A8,A8,A8,A16)") &
	  "  Config","   NLKey","  A Type","  B Type","   AtomA","   AtomB"," AtomB U",&
	  "      Rd"
	  Do i=1,size(neighbourListKey,1)
	    Do j=neighbourListKey(i,1),(neighbourListKey(i,1)+neighbourListKey(i,2)-1)
	      Write(11,"(I8,I8,I8,I8,I8,I8,I8,F16.8)") i,j,&
		  neighbourListI(j,1),neighbourListI(j,2),neighbourListI(j,3),&
		  neighbourListI(j,4),neighbourListI(j,5),&
		  neighbourListR(j)
	    End Do
	  End Do	
      Close(11)	  
