...
!Only calculate bulk modulus where there is a reference BM
    ...
    Do i=1,configCount 	  
      If(mpiProcessID.eq.configAtomsMap(i,23))Then
	    ...
	    End Do
		print *,i,mpiProcessID,configurationEC(i,1),configurationEC(i,2),configurationEC(i,3)
	  End If
    End Do
	Call MPI_synchProcesses() !Synchronise mpi processes at this point
	Call MPI_sendData2DDP(configurationEC,size(configurationEC,1),size(configurationEC,2))
	
	!Temp run on proc 0 only, sort distributing later
	
!Distribute data in array, 2D array, double precision
    !print *,mpiProcessID,"1",configurationEC(1,1),configurationEC(1,2),configurationEC(1,3)
    !print *,mpiProcessID,"2",configurationEC(2,1),configurationEC(2,2),configurationEC(2,3)
	
    !Call distributeData2DDP(configurationEC,size(configurationEC,1),size(configurationEC,2),23)	
	
	!If(mpiProcessID.eq.0)Then
	!  print *,configurationEC(1,1),configurationEC(2,1)
	!End If
...
!-------------------------------------
	...
	End Do
!-------------------------------------
!Elastic Constants Difference
!-------------------------------------
    Do i=1,configCount 
	  Do j=1,9
	    If(configurationEC(i,j).gt.(-2.0D20).and.configurationRefEC(i,j).gt.(-2.0D20))Then
	      rssTable(i,6) = rssTable(i,6) + &
		      1.0D0*(configurationRefEC(i,j)-configurationEC(i,j))**2
	    End If
	  End Do
	End Do	
!------------------------------------
...
!mpi variables
	...
	Real(kind=DoubleReal), Dimension(1:arraySizeW) :: dataArrayTemp, sendArray, recvArray
...
!-----------------------------------------    
      If(n.lt.4)Then
		print *,processID,n,dataArrayTemp(1),dataArrayTemp(2),dataArrayTemp(3)
	  End If
      If(processID.eq.0)Then
!RECV by master process	 
	    Do i=1,(processCount-1)
	      processFrom = i
		  tag = 1000 + i
          Call MPI_recv(recvArray,arraySizeW,&
		  MPI_double_precision,processFrom,tag,MPI_comm_world,status,error )
!Read specific worker slots into master array
          Do j=1,configCount
		    If(configAtomsMap(j,mapColumn).eq.i)Then
...
!SEND by worker processes
        ...
        Do i=1,(processCount-1)
          If(i.eq.processID)Then
!print *,processID,"Send"
            tag = 1000 + i	
            Call MPI_send(sendArray,arraySizeW,&
		    MPI_double_precision,processTo,tag,MPI_comm_world,error)
	      End If
        End Do
	  End If
