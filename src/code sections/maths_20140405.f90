...
! Spline Functions   
  ...
  Public :: FivePointInterpolationValue
! Matrix Functions  
...
!------------------------------------------------------------------------!   
  ...
  Function Spline(inputPoints,subsetStartIn,subsetLengthIn) RESULT (output) 
!force declaration of all variables
	Implicit None
!declare variables
	Integer(kind=StandardInteger) :: i,j,k
...
!optional variables
    Integer(kind=StandardInteger), optional :: subsetStartIn, subsetLengthIn
    Integer(kind=StandardInteger) :: subsetStart, subsetLength, subsetEnd
!sort out optional
  ...
  Function FivePointInterpolation(inputPoints) RESULT (coefficients)  
!force declaration of all variables
	Implicit None
!declare variables
	Integer(kind=StandardInteger) :: i,j,k
	...
	Real(kind=DoubleReal), Dimension( : ), Allocatable :: yMatrix, cMatrix, coefficients	
!If 5 sets of x,y points	
    If(size(inputPoints,1).eq.5)Then
	  inputPoints = 1.0D0*inputPoints
!Allocate matrices
	  Allocate(xMatrix(1:5,1:5))
	  Allocate(yMatrix(1:5))
	  Allocate(cMatrix(1:5))
...
!Copy to coefficients matrix
  ...
  Function FivePointInterpolationValue(inputPoints,x) RESULT (yArray)  
!force declaration of all variables
	Implicit None
!declare variables
	Integer(kind=StandardInteger) :: i,j,k
...
!Allocate matrices
	  ...
	  Allocate(yArray(0:2))
!Get coefficients
      coefficients = FivePointInterpolation(inputPoints)
!Calculate y, dy, d^2y
	  cIn = coefficients
	  yArray(0) = CalcPolynomial(cIn, x, 0)
	  cIn = coefficients
	  yArray(1) = CalcPolynomial(cIn, x, 1)
	  cIn = coefficients
	  yArray(2) = CalcPolynomial(cIn, x, 2)
    End If
  End Function FivePointInterpolationValue  
