...
! Interpolation and Regression Functions 
  ...
  Public :: FourPointInterpolation
  Public :: FivePointInterpolation
...
!return data as array
  ...
  Function FourPointInterpolation(inputPoints,x) RESULT (outputArray) 
!Explicit four point interpolation based on lagrange formula
!Outputs y, y', y'' and y''' in an array
!force declaration of all variables
	Implicit None
!declare variables
    Integer(kind=StandardInteger) :: i,n,k,j,ii,jj
	Integer(kind=StandardInteger) :: order, power, signVal, pointCount, dataPoint
    Real(kind=DoubleReal) :: x,y,dy,ddy,dddy,numerator,denominator,yVal,xVal,factor
	Real(kind=DoubleReal) :: tempCoefficient
	Real(kind=DoubleReal) :: x0,x1,x2,x3
	Real(kind=DoubleReal) :: y0,y1,y2,y3
	Real(kind=DoubleReal) :: r0,r1,r2,r3
    Real(kind=DoubleReal), Dimension( : , : ), Allocatable :: inputPoints
    Real(kind=DoubleReal), Dimension( : ), Allocatable :: coefficients
...
!set variables
    ...
    order = size(inputPoints,1)-1
	If(pointCount.eq.4)Then		!For 4 point only
!Allocate coefficients array	
	  Allocate(coefficients(0:3))
	  Allocate(outputArray(1:4))
!Initialise variables	  
	    ...
	    y3 = 0.0D0	
!set vars
	    x0 = 1.0D0*inputPoints(1,1)
	    y0 = 1.0D0*inputPoints(1,2)
	    x1 = 1.0D0*inputPoints(2,1)
	    y1 = 1.0D0*inputPoints(2,2)
	    x2 = 1.0D0*inputPoints(3,1)
	    y2 = 1.0D0*inputPoints(3,2)
	    x3 = 1.0D0*inputPoints(4,1)
	    y3 = 1.0D0*inputPoints(4,2)				
!make repeated set of coefficients
        r0 = 1.0D0*(y0/(1.0D0*(x0-x1)*(x0-x2)*(x0-x3)))
        r1 = 1.0D0*(y1/(1.0D0*(x1-x0)*(x1-x2)*(x1-x3)))
        r2 = 1.0D0*(y2/(1.0D0*(x2-x0)*(x2-x1)*(x2-x3)))
        r3 = 1.0D0*(y3/(1.0D0*(x3-x0)*(x3-x1)*(x3-x2)))
!make polynomial coefficients	
      coefficients(0)=-1.0D0*(r0*(x1*x2*x3)+r1*(x0*x2*x3)+&
	                  r2*(x0*x1*x3)+r3*(x0*x1*x2))
	  coefficients(1)=r0*(x1*x2+x1*x3+x2*x3)+r1*(x0*x2+x0*x3+x2*x3)+&
	                  r2*(x0*x1+x0*x3+x1*x3)+r3*(x0*x1+x0*x2+x1*x2)
	  coefficients(2)=-1.0D0*(r0*(x1+x2+x3)+r1*(x0+x2+x3)+&
	                  r2*(x0+x1+x3)+r3*(x0+x1+x2))
	  coefficients(3)=(r0+r1+r2+r3)
	  !print *,coefficients(3),coefficients(2),coefficients(1),coefficients(0)
!calculate values - y
      y = 0.0D0
	  Do n=0,3
...
!calculate values - dy
	  ...
	  End Do
!calculate values - ddy
	  Do n=0,1
	    coefficients(n) = (n+1)*coefficients(n+1) 
	  End Do
	  ddy = 0.0D0
	  Do n=0,1
	    ddy=ddy+1.0D0*coefficients(n)*x**n
	  End Do
	  outputArray(1) = y
	  outputArray(2) = dy
	  outputArray(3) = ddy
    Else
	  ...
	  outputArray(3) = 0.0D0
	End If
  End Function FourPointInterpolation  
  
  
    
  
  Function FivePointInterpolation(inputPoints,x) RESULT (outputArray) 
!Explicit five point interpolation based on lagrange formula
!Outputs y, y', y'' and y''' in an array
!force declaration of all variables
	Implicit None
!declare variables
    ...
    Real(kind=DoubleReal) :: x,y,dy,ddy,dddy,numerator,denominator,yVal,xVal,factor
	Real(kind=DoubleReal) :: tempCoefficient
	Real(kind=DoubleReal) :: x0,x1,x2,x3,x4
	Real(kind=DoubleReal) :: y0,y1,y2,y3,y4
	Real(kind=DoubleReal) :: r0,r1,r2,r3,r4
    Real(kind=DoubleReal), Dimension( : , : ), Allocatable :: inputPoints
    Real(kind=DoubleReal), Dimension( : ), Allocatable :: coefficients
    Real(kind=DoubleReal), Dimension( : ), Allocatable :: coefficientsOutput
    Real(kind=DoubleReal), Dimension( : ), Allocatable :: outputArray
...
!set variables
    pointCount = size(inputPoints,1)
    order = size(inputPoints,1)-1
	If(pointCount.eq.5)Then		!For 5 point only
!Allocate coefficients array	
	  Allocate(coefficients(0:3))
	  Allocate(outputArray(1:4))
!Initialise variables	  
...
!calculate values - ddy
    ...
    Else
	  ...
	  outputArray(3) = 0.0D0
	End If
  End Function FivePointInterpolation  
