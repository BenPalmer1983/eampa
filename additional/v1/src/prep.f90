...
!Synch processes at this point
	Call synchMpiProcesses()
!Neighbour List Analysis
	Call analyseNeighbourList()
!--------------------------
! Potential
!--------------------------
!Sort out potential
...
!Configuration rates	  
      ...
      configurationOptWeights(i,5) = 1.0D0
	  configurationOptWeights(i,6) = 1.0D0
...
!-------------------------- 
  ...
  Subroutine analyseNeighbourList()
!force declaration of all variables
	Implicit None	
!declare private variables
	Integer(kind=StandardInteger) :: i,j,k,y
	Real(kind=DoubleReal) :: minR, maxR
	Integer(kind=StandardInteger), Dimension(1:100) :: histogram
!open files
	open(unit=999,file=trim(trim(outputDirectory)//"/"//"output.dat"),&
	status="old",position="append",action="write")	
	open(unit=988,file=trim(trim(outputDirectory)//"/"//"nlseparation.dat"),&
	status="old",position="append",action="write")	
!histogram, min and max separation    
    histogram = 0
	Do i=1,size(neighbourListR)
!tally histogram	
	  y = floor(10.0D0*neighbourListR(i))
	  If(y.gt.100)Then
	    y = 100
	  End If
	  If(y.lt.1)Then
...
!min/max
      If(i.eq.1)Then
	    minR = neighbourListR(i)
	    maxR = neighbourListR(i)
	  Else	
		If(minR.gt.neighbourListR(i))Then
		  minR = neighbourListR(i)
		End If
		If(maxR.lt.neighbourListR(i))Then
		  maxR = neighbourListR(i)
		End If
	  End If 
	End Do
!write to output file
    If(mpiProcessID.eq.0)Then
      write(999,"(F8.4,A2,A6,E20.10,A8,E20.10)") ProgramTime(),"  ",&  	
	  "Rmin: ",minR,"  Rmax: ",maxR
	End If
!histogram of separations
    write(988,"(A22)") "Separation/Ang   Count"
    Do i=1,100
	  write(988,"(F14.6,A3,I5)") (i/10.0D0),"   ",histogram(i)
    End Do
    
	close(988)
	close(999)
  
  End Subroutine analyseNeighbourList 
