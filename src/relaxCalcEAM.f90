...
! Privacy of variables/functions/subroutines
  Private
! Public Subroutines
  Public :: relaxCalcEnergies
  Public :: relaxCalcEnergy
  Contains
! ---------------------------------------------------------------------------------------------------
  Subroutine relaxCalcEnergies()
...
! Loop through configurations
    ...
    Do configID=1,configCountRelax
      If(processMap(configID).eq.mpiProcessID)Then  ! build in more mpi options here
        Call relaxCalcEnergy(configID, configEnergy)
        configCalcEnergiesRelax(configID)=&
        (configEnergy/(1.0D0*configurationCoordsKeyRelax(configID,2)))
      End If
    End Do
  ...
  End Subroutine relaxCalcEnergies
! ---------------------------------------------------------------------------------------------------
  Subroutine relaxCalcEnergy(configID, configEnergy)
    ...
    End If
  End Subroutine relaxCalcEnergy
! ---------------------------------------------------------------------------------------------------
...
! -----------------
! Init variables
    rCutoff = bpCutoff ! replace with relaxCutoff
    pairEnergy = 0.0D0
    embeddingEnergy = 0.0D0
    configEnergy = 0.0D0
    nlStart = neighbourListKeyRelax(configID,1)
    nlLength = neighbourListKeyRelax(configID,2)
    nlEnd = neighbourListKeyRelax(configID,3)
    configStart = configurationCoordsKeyRelax(configID,1)
    configLength = configurationCoordsKeyRelax(configID,2)
    configEnd = configurationCoordsKeyRelax(configID,3)
    maxDensity = 0.0D0
! Init arrays
    Do i=1,configLength
    ...
    End If
!     neighbourListIRelax(n,1)  Atom A type
!     neighbourListIRelax(n,2)  Atom B type
!     neighbourListIRelax(n,3)  Atom A id
!     neighbourListIRelax(n,4)  Atom B id
! --------------------------------------------------
! Loop 1 - sum pair energy, pair force and electron density
! --------------------------------------------------
    Do n=nlStart,nlEnd
      aType = neighbourListIRelax(n,1)
      bType = neighbourListIRelax(n,2)
      aID_R = neighbourListIRelax(n,3)   ! ID Relatice (pos in this config)
      bID_R = neighbourListIRelax(n,4)
...
! If force-stress: store force from Pair Potential
        If(forceStressSwitch.eq.1)Then
          fX = -1.0D0*neighbourListCoordsRelax(n,10)*yArray(2)
          fY = -1.0D0*neighbourListCoordsRelax(n,11)*yArray(2)
          fZ = -1.0D0*neighbourListCoordsRelax(n,12)*yArray(2)
! Pair force on atom A
          configCalcForcesRelax(aID_A,1) = configCalcForcesRelax(aID_A,1) + fX
          configCalcForcesRelax(aID_A,2) = configCalcForcesRelax(aID_A,2) + fY
          configCalcForcesRelax(aID_A,3) = configCalcForcesRelax(aID_A,3) + fZ
! Pair force on atom B
          configCalcForcesRelax(bID_A,1) = configCalcForcesRelax(bID_A,1) - fX
          configCalcForcesRelax(bID_A,2) = configCalcForcesRelax(bID_A,2) - fY
          configCalcForcesRelax(bID_A,3) = configCalcForcesRelax(bID_A,3) - fZ
! Virial Stress pair force
          Do i=1,3  ! Loop through coordinate axis x,y,z
            j=1     ! force x
            k = 3*(i-1)+j  ! 1,1=1 1,2=2 1,3=3 2,1=4 ... 3,3=9
            stressNX = neighbourListCoordsRelax(n,6+i)*fX
            ...
            j=2     ! force y
            k = 3*(i-1)+j  ! 1,1=1 1,2=2 1,3=3 2,1=4 ... 3,3=9
            stressNY = neighbourListCoordsRelax(n,6+i)*fY
            ...
            j=3     ! force z
            k = 3*(i-1)+j  ! 1,1=1 1,2=2 1,3=3 2,1=4 ... 3,3=9
            stressNZ = neighbourListCoordsRelax(n,6+i)*fZ
...
! --------------------------------------------------
    Do i=1,configLength
      ...
      aID_A = aID_R+configStart-1   ! ID Absolute (pos in in all configs)
      aType = configurationCoordsIRelax(aID_A,1)
...
! Forces
          ...
          fX = -1.0D0*neighbourListCoordsRelax(n,10)*forceM
          fY = -1.0D0*neighbourListCoordsRelax(n,11)*forceM
          fZ = -1.0D0*neighbourListCoordsRelax(n,12)*forceM
! Pair force on atom A
          configCalcForcesRelax(aID_A,1) = configCalcForcesRelax(aID_A,1) + fX
          configCalcForcesRelax(aID_A,2) = configCalcForcesRelax(aID_A,2) + fY
          configCalcForcesRelax(aID_A,3) = configCalcForcesRelax(aID_A,3) + fZ
...
! Energy
    Do i=1,configCountRelax
      configCalcEnergiesRelax(i) = 0.0D20
    End Do
! Stresses
    Do i=1,configCountRelax
      Do j=1,9
...
! Forces
    Do i=1,configsAtomTotalRelax
      Do j=1,3
        configCalcForcesRelax(i,j) = 0.0D20
      End Do
    End Do
! Atom energies
    Do i=1,configsAtomTotalRelax
      Do j=1,2
        configAtomEnergyRelax(i,j) = 0.0D0
      End Do
    End Do
