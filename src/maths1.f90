...
! Declare variables - priv
    ...
    Logical, Optional :: forceCalcDervIn
    Logical :: forceCalcDerv
! optional arguments    
...
! set f'(x) and f''(x)
    ...
    Else
      Do i=startPoint,endPoint  ! Use input
        splinePoints(i,1) = inputNodes(i,1)
        splinePoints(i,2) = inputNodes(i,2)
        splinePoints(i,3) = inputNodes(i,3)
        splinePoints(i,4) = inputNodes(i,4)
      End Do
    End If
    !Do i=startPoint,endPoint
...
! Loop through data points
    ...
    Do i=1,numDataPoints
      If((i.eq.1).or.(x.ge.inputNodes(nodeKey+1,1).and.(nodeKey+1).lt.endPoint))Then
        ...
        coefficients = SplineAB(pointA, pointB)
        print *,nodeKey
        print *,pointA(1),pointA(2),pointA(3),pointA(4)
        print *,pointB(1),pointB(2),pointB(3),pointB(4)
        print *,coefficients(1),coefficients(2),coefficients(3),coefficients(4),coefficients(5),coefficients(6)
        print *,pointA(1),CalcPolynomial (coefficients, pointA(1), 0),&
        CalcPolynomial (coefficients, pointA(1), 1),&
        CalcPolynomial (coefficients, pointA(1), 2)
      End If
      dataPoints(i,1) = x
      dataPoints(i,2) = CalcPolynomial (coefficients, x, 0)
      dataPoints(i,3) = CalcPolynomial (coefficients, x, 1)
