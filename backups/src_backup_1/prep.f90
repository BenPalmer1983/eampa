...
!Internal subroutine variables
	...
	trialResidualSquareSum = 0.0D0	!Last calculation error measurement
...
!Transfer neighbour list data	
    do i=1,neighbourListCount
	  ...
	  neighbourListCoords(i,10) = &
	    (neighbourListCoords(i,7)*abs(neighbourListCoords(i,7)))/(1.0D0*neighbourListR(i)) !(xB-xA)(|xB-xA|/|B-A|)
	  neighbourListCoords(i,11) = &
	    (neighbourListCoords(i,8)*abs(neighbourListCoords(i,8)))/(1.0D0*neighbourListR(i)) !(yB-yA)(|yB-yA|/|B-A|)
	  neighbourListCoords(i,12) = &
	    (neighbourListCoords(i,9)*abs(neighbourListCoords(i,9)))/(1.0D0*neighbourListR(i)) !(zB-zA)(|zB-zA|/|B-A|) 
	enddo
!Deallocate arrays
...
!loop through reduced data points
	  Do i=1,numberOfPoints
		...
		yArray = PointInterpolationArr(eamData,x,3,potStart,potLength,"Y")
		y = yArray(1)
		dy = yArray(2)
!store data
...
!loop through reduced data points
	  Do i=1,numberOfPoints
		...
		yArray = PointInterpolationArr(eamDataReduced,x,3,potStart,potLength,"Y")
		y = yArray(1)
		dy = yArray(2)
!store data
...
!declare private variables
  ...
  End Subroutine varyEAMPotential
