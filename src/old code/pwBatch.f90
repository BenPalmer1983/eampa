...
Integer(kind=StandardInteger), Dimension(-101:101,1:5) :: aTally
Integer(kind=StandardInteger) :: i, b
!-----------------------
! Single "batch" file
!-----------------------
...
! Synch MPI processes    
    ...
    aTally = 0
    Do i=1,10000000
      a = RandomVaryPoint(1.5D0, 0.01D0, 0.1D0)
      b = Floor((a-1.5D0)/0.0001D0)
      aTally(b,1) = aTally(b,1) + 1
      a = RandomVaryPoint(1.5D0, 0.01D0, 0.2D0)
      b = Floor((a-1.5D0)/0.0001D0)
      aTally(b,2) = aTally(b,2) + 1
      a = RandomVaryPoint(1.5D0, 0.01D0, 0.3D0)
      b = Floor((a-1.5D0)/0.0001D0)
      aTally(b,3) = aTally(b,3) + 1
      a = RandomVaryPoint(1.5D0, 0.01D0, 0.4D0)
      b = Floor((a-1.5D0)/0.0001D0)
      aTally(b,4) = aTally(b,4) + 1
      a = RandomVaryPoint(1.5D0, 0.01D0, 0.5D0)
      b = Floor((a-1.5D0)/0.0001D0)
      aTally(b,5) = aTally(b,5) + 1
    End Do
    Do i=-101,101
      print *,(i/100.0D0),aTally(i,1),aTally(i,2),aTally(i,3),aTally(i,4),aTally(i,5)
    End Do
...
!---------------------------
    ...
    Do i=1,pwbNtyp
      write(103,"(A4,A2,F12.7,A2,A)") trim(pwbAtomicSpeciesC(i,1)),"  ",&
...
!---------------------------
    ...
    If(pwbFixedAtoms.eq.0)Then    !don't fix atoms, set force to 1 (default)
      Do i=1,pwbNat
        write(103,"(A4,A1,F12.7,A1,F12.7,A1,F12.7,A1)") &
    ...
    If(pwbFixedAtoms.eq.1)Then    !don't fix atoms, set force to 1 (default)
      Do i=1,pwbNat
        write(103,"(A4,A1,F12.7,A1,F12.7,A1,F12.7,A1,F12.7,A1,F12.7,A1,F12.7,A1)") &
            trim(pwbAtomLabelsWorking(i))," ",&
            pwbAtomCoordsWorking(i,1)," ",&
            pwbAtomCoordsWorking(i,2)," ",&
