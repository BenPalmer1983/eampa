...
!------------------------------------------------------------------------!
  ...
  Subroutine calcEval(eamKeySelected,eamDataSelected,rss)
!force declaration of all variables
	Implicit None	
...
!EAM data arrays
    Integer(kind=StandardInteger), Dimension( : , : ), Allocatable :: eamKeySelected	!reduced set of points optimised
    Real(kind=DoubleReal), Dimension( : , : ), Allocatable :: eamDataSelected           !reduced set of points optimised
!EAM data arrays (Clones)
    Integer(kind=StandardInteger), Dimension( : , : ), Allocatable :: eamKeySelectedClone	!reduced set of points optimised
    Real(kind=DoubleReal), Dimension( : , : ), Allocatable :: eamDataSelectedClone           !reduced set of points optimised	
!Clone eam potential arrays	
	eamKeySelectedClone = eamKeySelected
	eamDataSelectedClone = eamDataSelected
!Clear values
...
!Calculate energies, forces and stresses
    Call calcConfigEnergies(eamKeySelectedClone,eamDataSelectedClone)
!Bulk modulus
...
!Save difference to file
  ...
  Subroutine calcEvalFull(eamKeySelected,eamDataSelected,rss)
!force declaration of all variables
	Implicit None	
!declare private variables
	Integer(kind=StandardInteger) :: i,j,k
...
!Clone eam potential arrays	
	...
	eamDataSelectedClone = eamDataSelected
!Bulk modulus
    Call calcBulkModulii(eamKeySelectedClone,eamDataSelectedClone)
!Synch MPI processes
	Call synchMpiProcesses()
!Calc difference 
    rss = 0.0D0
...
!------------------------------------------------------------------------!
  ...
  Subroutine calcConfigEnergies(eamKeySelectedClone,eamDataSelectedClone)
!force declaration of all variables
	Implicit None	
!declare private variables
...
!Loop through configurations, and split over MPI processes
	Do i=1,configCount
	  ...
	  If(selectProcess.eq.mpiProcessID)Then
	    configurationEnergy(i) = CalcEnergy(i,eamKeySelectedClone,eamDataSelectedClone)
	  End If
	End Do
!-----------------------------------------------------------
...
Subroutine calcBulkModulii(eamKeySelected,eamDataSelected)
!force declaration of all variables
	Implicit None	
!declare private variables
...
!Only calculate bulk modulus where there is a reference BM
    Do i=1,size(configAtomsMap,1)
      If(mpiProcessID.eq.configAtomsMap(i,22))Then
		configurationBM(i) = CalcBulkModulus(i,eamKeySelected,eamDataSelected)
	  End If
    End Do
	Allocate(bufferArray(1:configCount))
    If(mpiProcessID.gt.0) Then
...
!------------------------------------------------------------------------!   
  ...
  Function CalcEnergy (configurationID,eamKeySelected,eamDataSelected) RESULT (energy)  
!force declaration of all variables
	Implicit None	
!declare private variables
...
!sum pair energy and density	
	  ...
	  If(mpiProcessID.eq.0)Then
	  print *,"Energy Calc started B"
	  print *,size(eamKeySelected,1)
	  print *,size(eamDataSelected,1)
	  End If
...
!Pair potential
	  If(neighbourListR(j).le.configurationRadiusCutoff(configurationID))Then	
		...
		eamKeySelected,eamDataSelected)  
...
!Convert stress to GPa
  ...
  Function CalcBulkModulus (configurationID,eamKeySelected,eamDataSelected) RESULT (bulkModulus) 
!force declaration of all variables
	Implicit None
!declare private variables
	Integer(kind=StandardInteger) :: i, j, k, l, n, configurationID, keyIJ, forceKey  
...
!Calculate energy
      configEnergyVal = CalcEnergy(configurationID,eamKeySelected,eamDataSelected)
  ...
  Function SearchPotentialPoint(atomA,atomB,potType,x,eamKeySelected,eamDataSelected)RESULT(yArray)
!force declaration of all variables
	Implicit None
!declare variables
...
!get the start point and length of potential data
    potStart = eamKeySelected(potKey,4)
    potLength = eamKeySelected(potKey,5)
!Return 0.0D0 if out of range
    If(x.ge.eamDataSelected(potStart,1).and.x.le.eamDataSelected((potStart+potLength-1),1))Then
      interpPointCount = eamInterpType + 1
!interpolate
	  yArray = PointInterpolationArr(eamDataSelected,x,interpPointCount,potStart,potLength)
	Else
	  yArray(1) = 0.0D0
	  yArray(2) = 0.0D0
	End If
