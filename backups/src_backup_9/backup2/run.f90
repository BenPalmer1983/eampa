...
!run
	...
	If(mpiProcessID.eq.0)Then
	  print *,"data sizea ",size(eamData,1)
	  End If
...
!declare private variables
	...
	Real(kind=DoubleReal) :: rss,x,a,b,xV,differenceRSS,tempDouble
!Temp testing vars
...
!choose type of calculation: eval energy forces bulkmodulus elasticconstants 
	If(calcRunType.eq.1)Then	!Calculates energy of configurations only (and forces/stresses) 
	  ...
	  !eamDataSet = 1 	 
	  If(mpiProcessID.eq.0)Then
	  print *,"data size ",size(eamData,1)
	  End If
	  Call calcConfigEnergies(eamKey,eamData)	!Calculate energies	  
	...
	If(calcRunType.eq.5)Then	!Evaluate	  
	  eamDataSet = 1 	
      !calcEval(eamKeySelected,eamDataSelected,rss)	  
	  Call calcEvalFull(eamKey,eamData,rss)  
	  Call calcOutput()	        !Print output to file
	  Call calcOutputForces()
	  If(mpiProcessID.eq.0)Then
	...
	If(calcRunType.eq.99)Then    !Test
	  ...
	  Call calcEval(eamKey,eamData,rss)
	  Call synchMPIProcesses()
	  If(mpiProcessID.eq.0)Then
	    print *,"Input Potential RSS:",trialResidualSquareSum  !Starting RSS
	  End If 
...
!declare private variables
	...
	Real(kind=DoubleReal) :: rss, startingRSS, bestRSS
