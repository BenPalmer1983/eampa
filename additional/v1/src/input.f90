...
!user input variables  
  ...
  Character(len=4) :: calcRunTypeText
...
!defect configurations
  Real(kind=DoubleReal) :: vacancyAlat, vacancyRcut, vacancyEnergy    
!pwb only options  
  Character(len=16) :: pwbType
...
!Variables - all run types       
  Public :: calcRunType, calcRunTypeText 
...
!Variables - Run calculation only
  Public :: configurationsFilePath,	configurationsFilePathTemp, saveConfigFile            	
...
!defect configurations
  Public :: vacancyAlat, vacancyRcut, vacancyEnergy    
...
!Set module/subroutine options
		...
		End If
		If(StrToUpper(fileRow(1:4)).eq."PRP6".or.StrToUpper(fileRow(1:4)).eq."CONF")then		!PREP Config File
!Set module/subroutine options
		  optionReadConf = 1
		End If
...
!read next line
	  ...
	  End If	  	  	  
	  If(StrToUpper(fileRow(1:15)).eq."#SAVECONFIGFILE")then
!read next line
	    Read(1,*,IOSTAT=ios) saveConfigFile
	  endif	  
...
!read next line
		...
		Read(buffere,*) eamOptWeights(5) !BM
		Read(bufferf,*) eamOptWeights(6) !EC
	  endif	 
!#weighting	type  
...
!Dft Input Settings
	  ...
	  End If	  
!-----------------------------
! Defect configurations to add
!-----------------------------	
      If(StrToUpper(fileRow(1:21)).eq."#VACANCYCONFIGURATION")Then
!read next line
        Read(1,"(A255)",IOSTAT=ios) fileRow
		Read(fileRow,*) buffera, bufferb, bufferc
		Read(buffera,*) vacancyAlat
		Read(bufferb,*) vacancyRcut
		Read(bufferc,*) vacancyEnergy
      End If
!-----------------------------
...
!-----------------------------	  
      If(StrToUpper(fileRow(1:8)).eq."#PWBTYPE")then
!read next line
	    Read(1,*,IOSTAT=ios) buffera		
		pwbType = StrToUpper(trim(buffera))
	  End If
...
!read next line
	  ...
	  End If 	
	  If(StrToUpper(fileRow(1:9)).eq."#PWBSTART")then
!read next line
	    Read(1,*,IOSTAT=ios) buffera	
        Read(buffera,*) pwbStart		
	  End If 		
	  If(StrToUpper(fileRow(1:7)).eq."#PWBEND")then
!read next line
	    Read(1,*,IOSTAT=ios) buffera	
        Read(buffera,*) pwbEnd		
	  End If 
...
!Add special configurations	  
	  If(vacancyAlat.gt.0.0D0.and.size(elements,1).gt.0)Then
	    write(301,"(A)") "#new"
	    write(301,"(A,E20.10)") "#LP ",vacancyAlat
	    write(301,"(A)") "#X 1 0 0"
	    write(301,"(A)") "#Y 0 1 0"
	    write(301,"(A)") "#Z 0 0 1"
	    write(301,"(A)") "#CC 1 1 1"
	    write(301,"(A)") "#F N"
	    write(301,"(A,E20.10)") "#EVAC ",vacancyEnergy
!256
		
		
	    write(301,"(A,E20.10)") "#end"
...
!close file	
	CLOSE(1) 	
!Copy configuration file if needed (e.g. PRP6)	
	If(saveConfigFile(1:4).ne."    ")Then
	  Open(UNIT=1,FILE=configurationsFilePath)
	  Open(UNIT=2,FILE=trim(trim(outputDirectory)//"/"//saveConfigFile)) 
      Do i=1,fileRows 
!Read in line
	    Read(1,"(A255)",IOSTAT=ios) fileRowBuffer	
	    write(2,"(A)") trim(fileRowBuffer)
	  End Do
	  CLOSE(1)
	  CLOSE(2)
	  !saveConfigFile
...
!close file	
	...
	If(calcRunTypeText.ne."PRP6")Then   !Skip reading in if PRP6 (prep config)
!allocate arrays
