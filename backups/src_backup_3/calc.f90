...
!------------------------------------------------------------------------!
  ...
  Subroutine calcEval(printOut, saveForces)
!force declaration of all variables
	Implicit None	
...
!write to output file
	  write(999,"(A33,F8.4,A3,I4)") "Evaluation ",&
	  ProgramTime(),"   ",globalCounter(2)
	  close(999)
	End If
!Calculate energies, forces and stresses
    Call calcConfigEnergies(printOut,saveForces)
!Bulk modulus

!Synch MPI processes
	Call synchMpiProcesses()
!Calc difference 
    Call calcDifference(printOut,trialRSS)
...
!Write energies to output file, if master process
	If(mpiProcessID.eq.0.and.printOut.eqv..true.)Then
	  ...
	  write(999,"(A34,I4)") "Configuration Calculation Details ",globalCounter(1)
	  write(999,"(A44,I4)") "--------------------------------------------"
	  Do i=1,configCount
...
!------------------------------------------------------------------------!
  ...
  Subroutine calcDifference(printOutIn, trialRSS)
!force declaration of all variables
	Implicit None	
!declare private variables
	Integer(kind=StandardInteger) :: i,j,k
...
!Check if there is a reference energy
	...
	trialRSS = energyDifference*100.0D0
!Force Difference
    ...
    If(calcForcesOnOff.eq.1)Then
	  ...
	  trialRSS = trialRSS + forceDifference
    End If
...
!get the start point and length of potential data
      ...
      interpPointCount = eamInterpType + 1
!interpolate
	  yArray = PointInterpolationArr(eamDataTrial,x,interpPointCount,potStart,potLength)
	End If
