...
!set start value for x
  ...
  Function CalcPolynomial (polyCoefficients, x, derivativeIn) RESULT (y)
...
!Declare variables
    ...
    Real(kind=DoubleReal), Dimension( : ) :: polyCoefficients
...
!Change coefficients (if required)
    If(derivative.gt.0)Then
      Do i=1,derivative
        ...
        End Do
        polyCoefficients(size(polyCoefficients)) = 0.0D0
      End Do
    End If
! Calculate
    y = 0.0D0
